...
void GSMenu::Init()
{
	...
	//play button
	texture = ResourceManagers::GetInstance()->GetTexture("play_button");
	...
	//exit button
	texture = ResourceManagers::GetInstance()->GetTexture("exit_button");
	...
	texture = ResourceManagers::GetInstance()->GetTexture("highscore_button");
	button = std::make_shared<GameButton>(model, shader, texture);
	...
	m_Text_gameName = std::make_shared< Text>(shader, font, "Super Mario", TEXT_COLOR::RED, 1.0);
